...
type MediaAttachment = ChatMessageAttachment & {
  createdAt: Date | null
  messageId: string
}
...
const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  const mediaAttachments = useMemo<MediaAttachment[]>(() => {
    return messages
      .flatMap((message) =>
  ...
  const handleOpenPreview = (attachment: MediaAttachment) => {
  ...
  return (
    <div className="fixed inset-0 z-[60]">
              ...
              >
                {activeParticipantId ? (
                  <>
                    ...
                    {isGalleryOpen && (
                      <div className="max-h-64 overflow-y-auto border-b border-slate-200 bg-white/75 px-4 py-3 backdrop-blur sm:max-h-72 md:max-h-80 md:px-6 md:py-4">
                        {mediaAttachments.length === 0 ? (
                          <p className="text-sm text-gray-500">ยังไม่มีไฟล์รูปภาพหรือวิดีโอในบทสนทนานี้</p>
                        ) : (
                          <div className="grid grid-cols-2 gap-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 md:gap-3">
                            {mediaAttachments.map((attachment) => (
                              <button
                                key={`${attachment.messageId}-${attachment.storagePath}`}
                                type="button"
                                onClick={() => handleOpenPreview(attachment)}
                                className="group relative block overflow-hidden rounded-xl border border-slate-200 bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                              >
                                {attachment.type === "image" ? (
                                  <img
                                    ...
                                    loading="lazy"
                                    className="h-24 w-full object-cover transition duration-300 group-hover:scale-105 sm:h-28 md:h-32"
                                  />
                                ) : (
                                  <>
                                    <video
      ...
      {previewAttachment && (
        <div
          className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/70 px-4 py-10"
          role="dialog"
          aria-modal="true"
          onClick={handleClosePreview}
        >
          <div
            className="relative flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-black/90 shadow-2xl"
            onClick={(event) => event.stopPropagation()}
          >
            <button
              type="button"
              onClick={handleClosePreview}
              className="absolute right-3 top-3 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/15 text-white transition hover:bg-white/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"
            ...
            <div className="flex flex-1 items-center justify-center bg-black">
              {previewAttachment.type === "image" ? (
                <img
                  ...
                  alt={previewAttachment.name || "ไฟล์รูปภาพ"}
            ...
            {(previewAttachment.name || previewAttachment.createdAt) && (
              <div className="border-t border-white/10 bg-black/60 px-5 py-4 text-sm text-white/80">
                <p className="font-medium text-white">{previewAttachment.name ?? "ไฟล์สื่อ"}</p>
                {previewAttachment.createdAt instanceof Date && (
                  <p className="text-xs text-white/70">
                    {new Intl.DateTimeFormat("th-TH", {
                      dateStyle: "medium",
                      timeStyle: "short",
                    }).format(previewAttachment.createdAt)}
                  </p>
                )}
              </div>
            )}
